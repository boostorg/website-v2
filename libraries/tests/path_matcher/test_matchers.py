...
def test_libs_path_to_latest_exact_db_path_exists(
    ...
    matcher = matcher_class(version)
    s3_key_spy = mocker.spy(matcher, "confirm_db_path_exists")
    pm = matcher.determine_match(test_path)

    s3_key_spy.assert_called_once_with(expected_s3_key)
...
def test_handoff(
    ...
    #  Same reason we use match_class(version).get_class_name() below rather than a string name for the class
    monkeypatch.setattr(
    ...
    assert match_result.matcher == matching_class(version).get_class_name()
...
def test_s3_archive_key_prefix(mocker, version):
    ...
    BasePathMatcher.confirm_s3_path_exists(test_path)
