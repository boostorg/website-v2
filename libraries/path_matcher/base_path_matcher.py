...
class BasePathMatcher(metaclass=ABCMeta):
    ...
    * ...Fallback - should be used when we're going return an index.htm(l) file in the latest library docs or otherwise
    ...
    class properties:
        has_equivalent: default 0, set to 1 if this class provides a direct equivalent path and no path translation
            is needed
    ...
    @property
    def is_index_fallback(self) -> bool:
        """
        Default False, set to True if this matcher accepts that the path may not actually exist.
        To be used when we are doing index fallbacks
        """
        return False

    def __init__(self, latest_version: Version):
    ...
    @staticmethod
    def confirm_s3_path_exists(path: str) -> bool:
        ...
        try:
            ...
            client.head_object(Bucket=bucket_name, Key=archive_key)
