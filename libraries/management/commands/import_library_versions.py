...
def command(min_release, release, token):
    ...
    for version in versions.order_by("-name"):
        # Get the gitmodules file, which contains the library repos to process
        ...
        for gitmodule in gitmodules:
            ...
            # See if we can get the library and add it easily.
            try:
                library = Library.objects.get(name__iexact=library_name)
                breakpoint()
                handle_library_version(version, library, [], updater)
                continue

            # If not, we have encountered a library that doesn't exist yet in the database
            # and we need to add it. This happens because of formatting changes in the gitmodules
            # file over time, or when libraries are split into separate libraries
...
def handle_library_version(version, library, maintainers, updater):
    ...
    click.secho(f"Saved library version {library_version}.", fg="green")
