...
from mailing_list.client import is_confirmed as mailman_is_confirmed
from mailing_list.client import subscribe as mailman_subscribe
from mailing_list.client import unsubscribe as mailman_unsubscribe
    ...
    def post(self, request):
        ...
        for list_id in to_unsubscribe:
            ...
            if sub and sub.status == SubscriptionStatus.PENDING:
                ...
                continue
            try:
                mailman_unsubscribe(email, list_id)
                UserMailingListSubscription.objects.filter(
                    user=request.user, list_id=list_id
                ).delete()
                unsubscribed.append(list_id)
...
class QuickSubscribeView(View):
    ...
    def _handle_anonymous(self, request, email, list_id):
        try:
            if mailman_is_confirmed(email, list_id):
                if _is_htmx(request):
                    return self._card(
                        request,
                        state="error",
...
class ConfirmSubscriptionView(View):
    def get(self, request, token):
        ...
        for list_id in list_ids:
            try:
                mailman_subscribe(email, list_id)
                if user is not None:
                    UserMailingListSubscription.objects.filter(
