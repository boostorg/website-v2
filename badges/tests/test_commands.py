...
def test_backfill_commits(plain_user):
    """Backfill grants the commits achievement from Commit data."""
...
def test_backfill_publisher(plain_user, super_user):
    """Backfill grants the publisher achievement from published entries."""
    now = timezone.now()
    baker.make(
        "news.Entry",
        author=plain_user,
    ...
    call_command("backfill_achievements", "--source", "publisher")

    assert UserBadge.objects.filter(
        user=plain_user, badge__achievement__slug="publisher"
    ).exists()
...
def test_recalculate_revokes_badges_whose_achievements_are_gone(plain_user):
    ...
    UserAchievement.objects.create(
        user=plain_user, achievement=achievement, source_type="manual"
    )
